...
//sources: TA Tiantian
//collaborators: Christopher Payumo
public class Eval {
    ...
    static double value_roll_hand (State s, int depth) {   
    	...
    	double roll_Chance;
    	double roll_val;
		// loops through all possible three dices
				    ...
				    if (d2 != DieFace.invalid) {
						for (DieFace d3 : DieFace.values()) {
						    ...
						    if (d3 != DieFace.invalid) {
								//uses roll_val to calculate the expected value of the roll state
								roll_val = Eval.value_rolled_hand(s, depth);
						    	//updates s.roll
								s.roll(d1, d2, d3);
						    	//uses roll_chance to find out the probability from the roll
						    	roll_Chance = s.rollProb(d1, d2, d3);
								// now update val to be the combination of roll_chance and roll_val
								val = val + (roll_val * roll_Chance);
						    }
						}
				    }
				}
    ...
    static public double heuristic (State s) {
    	...
    	//value of an uneaten brain when there are blasts
		float blasts_scale = (float) 0.45;
		//value of an uneaten brain when there are no blasts
		float no_blasts_scale = (float) 0.50;
		// the probability of a win at the end of the game
		double near_win_scale = 0.80;
		// eff brains eaten by computer player
		int eff_comp_brains_eaten = s.comp_brains_eaten;
		// eff brains eaten by user
		int eff_user_brains_eaten = s.user_brains_eaten;
		
		//find out the number of brains that will be eaten at the end of this turn
		//based on the probability stated above
		if (s.current_player == Turn.computer) { //when it's the computer's turn
			//check if we are still rolling the dice
		    if (s.current_choice == Choice.roll) {
		    	//when the roll number is 0, it means that there are no blasts.
		    	//thus, we calculate the number of brains eaten without blast
				if (s.blasts_collected <= 0) {
					eff_comp_brains_eaten = eff_comp_brains_eaten + Math.round(no_blasts_scale * ((float) s.brains_collected));
				} 
				//when the roll number is greater than 0, it means that there are blasts.
		    	//thus, we calculate the number of brains eaten with blast
				else if (s.blasts_collected > 0){
					eff_comp_brains_eaten = eff_comp_brains_eaten + Math.round(blasts_scale * ((float) s.brains_collected));
				}
			} 
		    //this simply means that it's not a dice roll, 
		    //we calculate the number of brains eaten without the consideration of blasts or not
			else if (s.current_choice != Choice.roll){
		...
		//when it's the player's turn
			...
			//check if we are still rolling the dice
		    if (s.current_choice == Choice.roll) {
		    	//when the roll number is 1, it means that there are no blasts.
		    	//thus, we calculate the number of brains eaten without blast
				if (s.blasts_collected <= 1) {
					eff_user_brains_eaten = eff_user_brains_eaten + Math.round(no_blasts_scale * ((float) s.brains_collected));
				} 
		    	//when the roll number is greater than 1, it means that there are blasts involved.
		    	//thus, we calculate the number of brains eaten with blast
				else if (s.blasts_collected > 1){
					eff_user_brains_eaten = eff_user_brains_eaten + Math.round(blasts_scale * ((float) s.brains_collected));
				}
			} 
		    //this simply means that it's not a dice roll, 
		    //we calculate the number of brains eaten without the consideration of blasts or not
		...
		// Assign a heuristic evaluation value based on the number of
		// effective brains eaten ...
		if ((eff_comp_brains_eaten > State.brains_to_win) || (eff_user_brains_eaten > State.brains_to_win)) {
		    ...
		    // This means the player is winning
			else {
		    ...
		    double score_diff = (double) (eff_comp_brains_eaten - eff_user_brains_eaten);
		    value = (score_diff / State.brains_to_win) * State.win_payoff;
		}
		
		//uses temp_val to compare and find the higher value before the current value and the negative of win off value
		double temp_val = Math.max((- State.win_payoff), value);
		//the final value compares the win off value and the temp_value and gets the smaller value
		value = Math.min(State.win_payoff, temp_val);
