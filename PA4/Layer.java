...
public class Layer {
	...
	public void computeOutputDelta() {
		...
		//error is the difference between the target vector layer and the actual vector layer
		Vector err = this.targ.difference(this.act);
		//the derivative between the min and max
		Vector dv = net.derivative(min,max);
		for(int i = 0; i < n; i++){
			//multiply the derivative with the error
			delta.set(i, err.get(i)*dv.get(i));
		}
	...
	public void computeHiddenDelta() {
		...
		//new error vector
		Vector err = new Vector(n, a);
		//loops thru projection to all the outputs
		for(Projection pro : outputs){
			//add all the outputs together
			err = err.sum(pro.W.transpose().product(pro.output.delta));
		}
		//now calculate the derivative with min and max values
		Vector deriv = net.derivative(min,max);
		
		for(int i = 0; i < n; i++){
			//error multiply by the derivative
			//computes the final value by multiplying many times
			delta.set(i, err.get(i)*deriv.get(i));
		}
